...
def generate_campaign_hashtags(campaign_name: str, product: str, target_audience: str, objective: str) -> List[str]:
    prompt = f"""
Create 5 creative hashtags for an Indian campaign based on the following details:
...
Objective: {objective}
Each hashtag should be short, catchy, and incorporate Hinglish elements to resonate with Indian audiences.
"""
    response_data = generate_text(prompt, "llama3-70b-8192", Hashtag.model_json_schema())
    return Hashtag(**response_data).hashtags
...
def main():
    ...
    try:
        ...
        hashtags = generate_campaign_hashtags(campaign_name, product, target_audience, objective)
